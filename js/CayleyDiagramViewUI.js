...
```javascript
 */
export function addGestures(cayleyDiagramView) {
...
function showTooltipOrReset(cayleyDiagramView) {
    ...
    ;
    cayleyDiagramView.viewModel.model
    ...
    function getObjectsAtEventLocation(event) {
 ...
 * redraw -- move object in three dimensions based on the two-dimensional screen location of the cursor
 ```js
 */
function moveObjects(cayleyDiagramView) {
    ...
    cayleyDiagramView.container
        .insertAdjacentHTML('beforeend', `<div id="cayley-diagram-view-ui-mask" class="modal" style="top: ${graphicTop}px"></div>`);
    const modalMask = document.getElementById('cayley-diagram-view-ui-mask');
    ...
    // move element
    recognizeDragAndDrop(modalMask, (startEvent, previousEvent, currentEvent, isDrop) => {
...
function getPickedObject(cayleyDiagramView, event) {
    ...
    const linesAndSpheres = [...cayleyDiagramView.arrows, ...cayleyDiagramView.nodes];
...
class Customizable {
    ...
    object3D;
    originalColor;
    constructor(cayleyDiagramView, raycaster, object3D) {
...
class Arrow extends Customizable {
    ...
    redraw(currentLocation, _isDragStart) {
        const arrow = this.object3D.userData.arrow;
        const line = this.cayleyDiagramView.arrows
            .find((line) => line.userData.arrow == arrow);
        // update raycaster with new event location
        this.raycaster.setFromCamera(currentLocation, this.cayleyDiagramView.camera);
        const pickLine = new THREE.Line3(this.raycaster.ray.origin, this.raycaster.ray.origin.clone().addScaledVector(this.raycaster.ray.direction, 100));
        ...
        const pick = arcPlane.intersectLine(pickLine, new THREE.Vector3());
        ...
        const pickSign = Math.sign(new THREE.Vector3().crossVectors(end.clone().sub(start), thirdPoint.clone().sub(end))
    ...
    get color() {
        const currentColor = this.object3D.userData.arrow.color;
        return currentColor;
    }
    set color(color) {
        ;
        this.object3D.userData.arrow.color = color;
...
class Node extends Movable {
    ...
    get color() {
        ...
        const currentColor = this.cayleyDiagramView.color_highlights[sphereIndex];
        return currentColor;
    }
    set color(color) {
        const sphereIndex = this.object3D.userData.node.element;
    ...
    // move sphere
    moveObject3DTo(position) {
        this.cayleyDiagramView.moveSphere(this.object3D, position);
    }
}
/*
```
...
class Chunk extends Movable {
    ...
    get color() {
        const currentColor = '#' + this.object3D.material.color.getHexString();
        return currentColor;
    }
    set color(color) {
        const object3D = this.object3D;
        const newMaterial = object3D.material.clone();
        newMaterial.color.set(color);
        object3D.material = newMaterial;
    }
    // move chunk
    moveObject3DTo(position) {
        this.cayleyDiagramView.moveChunkTo(this.object3D, position);
    }
}
...
```
*/
//# sourceMappingURL=CayleyDiagramViewUI.js.map