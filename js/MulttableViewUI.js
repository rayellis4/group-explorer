...
function addSelect(multtableViewModel /*: MulttableViewModel */) {
    recognizeSelect(multtableViewModel.canvas, (event) => {
        ...
        if (rowXcol != undefined && !hasLabel) {
            ...
            const tooltip = `<div id="node-label">
...
function dragStart(event /*: ClientLocation */, multtableViewModel /*: MulttableViewModel */, dragImage /*: ?HTMLImageElement */) {
    ...
    const style = `position: absolute;
          ...
          opacity: 0.75;`;
    const dragImageTemplate = `<img id="drag-image" src="${multtableViewModel.canvas.toDataURL()}" swapping="${swapping}"
    ...
    if (dragImage != null) {
        ...
        if (rowXcol != undefined) {
            ...
            if ((swapping == 'row' && (rowXcol.col == 0 && rowXcol.row != 0 && rowXcol.row != start))
                ...
                multtableViewModel.model.$touch('elements');
            }
        }
        dragImage.remove();
    }
