...
function dataToGroup(data, contentType = '') {
    let group;
...
function decorateGeneratedGroup(group, generatorNames, relators, generatorElements) {
    const namePrefix = `A Generated Group of Order ${group.order}`;
    const nameSuffix = Math.max(...GroupRegistry.getGroupsByOrder(group.order)
            ...
            window.fetch(groupURL)
                ...
                try {
                    if (response.ok) {
                        ...
                        if (remoteGroup == null) {
                            reject(new Error(`Error reading ${groupURL}: unknown content type ${contentType}`, { cause: response }));
                        }
                        else {
...
const updatedGroupURLs = [];
const deletedGroupURLs = [];
...
export async function updateGroups(manifest) {
    await loadLibrary();
    ...
    const fetchURLs = new Set(manifest.filter((entry) => typeof entry === 'string'));
