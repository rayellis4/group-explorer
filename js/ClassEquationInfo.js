...
function makeClassEquationContent(group) {
    ...
    htmlFragments.push(`<div class="indent-children">The <a href="./help/rf-groupterms/index.html#class-equation">class equation</a>
    ...
    if (group.isAbelian) {
        htmlFragments.push(`<div>Each of the ${group.order} elements in the group is in a conjugacy class of its own,
            because the group is abelian.</div>`);
    }
    else {
        htmlFragments.push(`<div class="indent-children">The <a href="./help/rf-groupterms/index.html#conjugacy-classes">conjugacy classes</a>
            used in the class equation are listed here:
            <ul>`, ...group.conjugacyClasses.map((conj) => ['<li>', conj.toArray().map((el) => group.representation[el]).join(', '), '</li>'].join('')), `</ul>
          </div>`);
    }
    htmlFragments.push(`<div>To see a visual representation of this class equation, click one of the following links:
          <br>
          Show me by
...
function showAsSheet(group, type) {
    ...
    const W = Math.min(2 * window.innerHeight / 17, 0.5 * window.innerWidth / (numCols + (numCols - 1) * opFrac));
    const H = W;
    const opW = opFrac * W;
    ...
    for (let i = 0; i < fakeN; i++) {
        ...
        if (fakeIndex == -1) {
            sheetElementsAsJSON.push({ className: 'TextElement', x: colX, y: numY, w: W, h: numH,
                text: '...', fontSize: `${fontSize}px`, alignment: 'center', opacity: 0 }, { className: 'TextElement', x: colX, y: vizY, w: W, h: H,
        ...
        sheetElementsAsJSON.push({ className: 'TextElement', x: opX, y: numY, w: opW, h: numH,
            text: (fakeIndex < n - 1) ? '+' : '=',
            fontSize: `${fontSize}px`, alignment: 'center', opacity: 0 }, { className: 'TextElement', x: opX, y: vizY, w: opW, h: H,
            text: (fakeIndex < n - 1) ? '+' : '=',
...
//# sourceMappingURL=ClassEquationInfo.js.map