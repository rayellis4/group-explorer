...
function makeCyclicInfoContent(group) {
    ...
    if (group.isCyclic) {
        ...
        htmlFragments.push(`<div>${group.name} is <a href="./help/rf-groupterms/index.html#cyclic-group">cyclic</a>;
            an element that generates the group is ${group.representation[generator]}.</div>`);
    }
    else {
        htmlFragments.push(`<div>${group.name} is not <a href="./help/rf-groupterms/index.html#cyclic-group">cyclic</a>;
            no element generates the whole group.</div>`);
    }
    htmlFragments.push(`<button class="gap-compute" data-GAP="checking if a group is cyclic">Compute this in GAP</button>
      </details>`);
    return htmlFragments.join('');
}
//# sourceMappingURL=CyclicInfo.js.map