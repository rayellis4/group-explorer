...
function formatZmnInfo(group) {
    ...
    const [m, n, _] = factors.reduce(([fac1, fac2, prev], el) => {
        if (el >= prev) {
            fac1 *= el;
            prev = el;
        }
    ...
    if (factors.length == 1) {
        htmlFragments.push(`<div>A group of the form ℤ<sub>mn</sub> is isomorphic to the product group
        ...
        for (let m = 2; m <= Math.sqrt(group.order); m++) {
            if (group.order % m == 0) {
                ...
                htmlFragments.push(`<div><a href="" data-action="show${isZmn ? '' : 'No'}ZmnIsomorphismSheet(group, ${m},${n})">Click here</a> to see
        ...
        htmlFragments.push(`<div>A group of the form ℤ<sub>mn</sub> is isomorphic to the product group
             ...
             is isomorphic to ℤ<sub>${m * n}</sub>.</div>`, `<div><a href="" data-action="show${isZmn ? '' : 'No'}ZmnIsomorphismSheet(group, ${m},${n})">Click here</a> to see
             ${isZmn ? 'an illustration of' : ''} why ℤ<sub>${m * n}</sub> is ${isZmn ? '' : 'not'}
             isomorphic to ℤ<sub>m</sub> × ℤ<sub>n</sub>.</div>`);
    }
...
function showZmnIsomorphismSheet(group, m /*: groupElement */, n /*: groupElement */) {
    ...
    const W = Math.min(4 * window.innerHeight / 17, (window.innerWidth - panelWidth) / 4 // 3 viz + 2 half-gaps = 4W
    );
    const H = W;
    const gap = W / 2;
...
//# sourceMappingURL=ZmnInfo.js.map