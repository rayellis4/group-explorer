...
class View {
    ...
    async showStoredSheets() {
        ...
        if (sheetChoices.length === 0) {
            ...
            sheetChoices.forEach((sheetChoice) => {
                sheetList.insertAdjacentHTML('beforeend', `<li data-action="showStoredSheetMenu(event, '${GEUtils.escapeHTML(sheetChoice)}')">${sheetChoice}</li>`);
            });
        }
    }
    ...
    async showNameSheetDialog(location, sheetName, sheetContent) {
        ...
        const createSheetDialogHTML = `<div id="new-sheet-dialog" data-action="() => {}" class="flex-v"
    ...
    showImportSheetDialog(location) {
        const importSheetDialogHTML = `<div id="import-sheet-dialog" class="flex-v" style="min-height: 15em; min-width: 60ch; overflow: hidden">
    ...
    async showBackupSheetsDialog(location) {
        ...
        for (const sheet of allSheets) {
            backupSheetsDialogHTML.push(`<input type="checkbox" name="${GEUtils.escapeHTML(sheet)}" checked>
              <label for="${GEUtils.escapeHTML(sheet)}">${sheet}</label><br>`);
        }
        backupSheetsDialogHTML.push(`</div>
        ...
        const getCheckedSheetsJSONString = async () => {
            ...
            for (const inx in checkedSheetNames) {
                if (checkedSheetsJSON[inx].status === 'fulfilled') {
                    checkedSheetsJSON[inx].value.sheetName = checkedSheetNames[inx];
    ...
    async showRestoreSheetsDialog(location) {
        const restoreSheetsDialogHTML = `<div id="restore-sheets-dialog" class="flex-v" style="min-height: 13em; min-width: 50ch; width: 60ch; overflow: hidden">
            <style>
...
//# sourceMappingURL=SheetControl.js.map