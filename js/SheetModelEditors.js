...
class MorphismEditor extends SheetElementEditor {
    constructor(morphismElement, location) {
        ...
        this.updatePreview();
        addHighlightControl(document.getElementById('morphism-domain-highlight-control'), morphismElement.source.viewElement.highlightModelProxy);
        addHighlightControl(document.getElementById('morphism-codomain-highlight-control'), morphismElement.destination.viewElement.highlightModelProxy);
    }
    updateModelElement() {
    ...
    fillDefiningPairs() {
        ...
        if (this.modelElement.mapping.definingPairs.length === 0) {
            ...
            tbody.innerHTML = '';
            this.modelElement.mapping.definingPairs.forEach(([domainElement, codomainElement]) => {
                const definingPairRow = `<tr id="defining-pair-${domainElement}">
    ...
    updatePreview() {
        ...
        if (previewTable.style.display !== 'none') {
            ...
            this.modelElement.mapping.fullMapping.forEach((codomainElement, domainElement) => {
                const morphismPreviewRow = `<tr>
                      <td>${this.modelElement.source.group.representation[domainElement]}</td>
                      <td>${this.modelElement.destination.group.representation[codomainElement]}</td>
                   </tr>`;
