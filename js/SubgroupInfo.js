        ...
        conjugateSubgroupClasses.forEach((classSubgroupsBitSet, classIndex) => {
            ...
            if (conjugacyClassSubgroups.length > 1) {
                membershipCount.forEach((count, el) => {
                    if (count > 1)
                        highlights[1][el] = 'white';
                    if (count === conjugacyClassSubgroups.length)
                        highlights[0][el] = '#b0b0b0';
                });
            }
            sheetElementsAsJSON.push({
                className: type,
...
function showEmbeddingSheet(group, indexOfH /*: number */, type /*: VisualizerType */) {
    ...
    const W = Math.min(5 * window.innerHeight / 17, (window.innerWidth - panelWidth) / 2.5 // 2 viz + 1 half-gap = 2.5W
    );
    const Hv = W;
    const gap = W / 2;
...
function showQuotientSheet(group, indexOfN /*: number */, type /*: VisualizerType */) {
    ...
    const panelWidth = SheetModel.sheetPanelWidth();
    const W = Math.min(4 * window.innerHeight / 17, (window.innerWidth - panelWidth) / 7 // 5 viz + 4 half-gaps = 7W
    );
    const H = W;
    const gap = W / 2;
    ...
    function shrink(order, x, y, w, h) {
        const factor = 0.5 * (1 + order / group.order), hMargin = (w - factor * w) / 2, vMargin = (h - factor * h) / 2;
        return {
            x: x + hMargin, y: y + vMargin, w: factor * w, h: factor * h
        };
    }
    const col1 = 'hsl(60, 100%, 60%)', col2 = 'hsl(240, 100%, 80%)', col3 = '', col4 = 'hsl(120, 100%, 50%)', col5 = 'hsl(120, 90%, 85%)', loc1 = shrink(1, L, vizY, W, H), loc2 = shrink(libraryN.order, L + W + gap, vizY, W, H), loc3 = shrink(group.order, L + 2 * W + 2 * gap, vizY, W, H), loc4 = shrink(libraryQ.order, L + 3 * W + 3 * gap, vizY, W, H), loc5 = shrink(1, L + 4 * W + 4 * gap, vizY, W, H), high1 = Array(1).fill(col1), high2 = Array(libraryN.order).fill(col2), high3 = Array(group.order).fill(col3), high4 = Array(libraryQ.order).fill(col3), high5 = Array(1).fill(col5);
...
//# sourceMappingURL=SubgroupInfo.js.map