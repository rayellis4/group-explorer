...
function setArrowColors(arrows, passedArrowGenerators) {
    if (passedArrowGenerators == null) {
        ...
        const arrowGeneratorMap = new Map(passedArrowGenerators.map((arrowGenerator) => [arrowGenerator.generator, arrowGenerator]));
...
const ARROW_COLORS = ['#89b910', '#b79100', '#f58231', '#469990', '#808000', '#007700', '#0d0db0', '#990000'];
export function nextArrowColor(colorsUsed = []) {
    let nextColor;
...
function createArrows(group, chunkTree, generators, rightMultiply) {
    ...
    const isCurved = (startNode, endNode) => {
        ...
        if (drawCurved
            ...
            const startNodeParentIndex = commonChunk.chunks.findIndex((child) => child.leaves.some((node) => node == startNode));
            const endNodeParentIndex = commonChunk.chunks.findIndex((child) => child.leaves.some((node) => node == endNode));
    ...
    const getThirdPointForCurvedLayoutStrategy = (node1, node2) => {
        ...
        const commonChunkNormal = new THREE.Vector3()
            ...
            .normalize();
        const commonPlane = new THREE.Plane().setFromCoplanarPoints(commonChunk.position, position1, position2);
...
class CurvedLayoutStrategy extends AbstractLayoutStrategy {
    ...
    constructor(generator, direction, nesting_level) {
        super(generator, direction, nesting_level);
        this.positionTransform = positionTransforms[direction];
    }
...
class CircularLayoutStrategy extends CurvedLayoutStrategy {
    ...
    layoutNodes(chunk) {
        ...
        const transform = (new THREE.Matrix4()).makeScale(...new THREE.Vector3(...Array.from({ length: 3 }, (_, inx) => (this.directionIndex == inx) ? 1 : scale)).toArray());
...
class Chunk {
    ...
    constructor(children, strategy) {
        if (children[0] != null) {
            if (Object.getPrototypeOf(children[0]) === Object.prototype) {
                this.leaves.push(...children);
            }
            else {
                this.chunks.push(...children);
            }
        }
        if (strategy != null)
    ...
    getNodeAncestry(node) {
        const ancestors = (this.isLeaf)
            ? [this]
            : [...this.chunks.find((child) => child.allChildNodes.includes(node)).getNodeAncestry(node), this];
        return ancestors;
    }
    setPositionFromChildren() {
        ...
        const [xMin, xMax, yMin, yMax, zMin, zMax] = allChildNodes.reduce(([xMin, xMax, yMin, yMax, zMin, zMax], node) => {
            return [
...
function generateTree(G, strategies) {
    function populateTree(remainingStrategies, elementsUsed) {
        ...
        if (remainingStrategies.length == 0) {
            ...
            const nodeTreeMultiply = (g, chunk) => {
                if (chunk.isLeaf) {
    ...
    function sortTreeByNestingLevel(nodeTree) {
        let changed = false;
        if (!nodeTree.isLeaf) {
            if (nodeTree.strategy.nesting_level <
                nodeTree.chunks[0].strategy.nesting_level) {
                changed = true;
                ...
                for (let i = 0; i < nextLength; i++) {
                    ...
                    nodeTree.chunks.push(new Chunk(tmp, topStrategy));
                }
            }
            // visit the rest of the tree and accumulate changes there
            changed ||=
                nodeTree.chunks.reduce((anyChanged, child) => sortTreeByNestingLevel(child) || anyChanged, false);
        }
        return changed;
    }
...
function getWidth(nodes, directionIndex) {
    if (!Array.isArray(nodes))
        return 0;
    const [min, max] = nodes.reduce(([min, max], node) => [Math.min(min, node.position.getComponent(directionIndex)),
        Math.max(max, node.position.getComponent(directionIndex))], [Number.MAX_VALUE, Number.MIN_VALUE]);
    return max - min;
}
...
function normalizeScene(chunkTree) {
    ...
    const scaleArray = new THREE.Vector3()
        ...
        if (originalSize > .01 && originalSize < 0.5) {
            ...
            const hasUnsafeStrategy = allCurvedStrategies.some((strategy) => unsafeDirections.includes(strategy.direction));
...
function generateStrategy(G) {
    ...
    if (G.order % 2 == 0) {
        ...
        if (N != null) {
            const H = G.subgroups
                .find((H) => !H.isNormal && H.order == 2 && !N.members.isSet(H.generators.first()));
            if (H != null) {
    ...
    const getComplement = (N) => G.nontrivialProperSubgroups.find((H) => G.closure(BitSet.union(N.members, H.members)).popcount() == G.order);
        ...
        case 2:
            ...
            if (element_orders[ordered_gens[0]] == G.order / 2 && G.order > 4) {
                if (element_orders[ordered_gens[1]] == 2) {
                    strategies.push({ generator: ordered_gens[1], layout: 'linear', direction: 'X', nestingLevel: 0 }, { generator: ordered_gens[0], layout: 'rotated', direction: 'XY', nestingLevel: 1 }); // see D_4
                }
                else {
                    strategies.push({ generator: ordered_gens[1], layout: 'circular', direction: 'XY', nestingLevel: 0 }, { generator: ordered_gens[0], layout: 'linear', direction: 'Z', nestingLevel: 1 }); // see Q_4
                }
            }
            else {
                ...
                strategies.push({ generator: ordered_gens[0], layout: 'linear', direction: first_gen_dir, nestingLevel: 0 }, { generator: ordered_gens[1], layout: 'linear', direction: second_gen_dir, nestingLevel: 1 }); // see S_4
            }
            break;
        case 3:
            strategies.push({ generator: generators[0], layout: 'linear', direction: 'X', nestingLevel: 0 }, { generator: generators[1], layout: 'linear', direction: 'Y', nestingLevel: 1 }, { generator: generators[2], layout: 'linear', direction: 'Z', nestingLevel: 2 });
            break;
        case 4:
            strategies.push({ generator: ordered_gens[0], layout: 'linear', direction: 'X', nestingLevel: 0 }, { generator: ordered_gens[1], layout: 'linear', direction: 'X', nestingLevel: 1 }, { generator: ordered_gens[2], layout: 'linear', direction: 'Y', nestingLevel: 2 }, { generator: ordered_gens[3], layout: 'linear', direction: 'Z', nestingLevel: 3 });
            break;
    }
    return strategies;
}
...
function generateAbelianStrategy(G) {
    ...
    if (generators.length == 1) {
        ...
        strategies.push({ generator: generators[0], layout: 'linear', direction: 'X', nestingLevel: 0 }, { generator: generators[1], layout: 'rotated', direction: 'XY', nestingLevel: 1 });
    }
    else {
...
//# sourceMappingURL=CayleyDiagramGenerator.js.map