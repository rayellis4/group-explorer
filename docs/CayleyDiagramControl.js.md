...
class ViewModel {
   ...
   #refineStrategies (newStrategies /*: Array<StrategyParameters> */) /*: Array<StrategyParameters> */ {
      ...
      if (elementsGenerated.popcount() != this.group.order) {
         ...
         strategies.push(
            {generator: newGenerator, layout: 'linear', direction: newDirection, nestingLevel: strategies.length}
         )
      }
...
class DiagramChoice extends View {
   ...
   showDiagramChoices () {
      makeMockSelect(this.diagramSelect, this.choices)
         .then(
            (choice) => this.viewModel.chooseDiagram(choice),
            () => {}
         )
   }
...
class Generator extends View {
   ...
   showAxisMenu (clickLocation /*: NumberLocation */, strategyIndex /*: number */) {
      ...
      const currentOrder = this.group.elementOrders[this.viewModel.strategyParameters[strategyIndex].generator]
      const previousOrder = (strategyIndex == 0) 
         ? 1
         : this.group.elementOrders[this.viewModel.strategyParameters[strategyIndex - 1].generator]
      const curvable = (currentOrder / previousOrder) > 2
...
// utility function clearArrowList() to remove all arrows from list (called during reset)
   ...
   update () {
      ...
      this.viewModel.arrowGenerators?.forEach(({generator, color}) => {
         const listItem =
            `<li data-arrow="${generator}" data-color="${color}" data-action="this.selectArrow(${generator})">
