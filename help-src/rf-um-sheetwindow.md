...
viewing at a later time, it can import and export sheets as strings so you
can share them or store them elsewhere, and you can backup and restore the
your stored sheets as files on your computer. The stored sheets will persist
...
To rename a stored sheet right-click [tap] on the sheet in the stored sheet
display and select "Rename sheet" from the menu. This will bring up a dialog box
into which you can enter a new sheet name. The name is treated as HTML, so you
can include unicode, formatting for subscripts and superscripts, etc. Duplicate
names are not allowed.

#### Export, Import, Backup, and Restore

*Group Explorer* provides two general ways to save and transfer your stored
sheets: *Backup/Restore* and *Export/Import*. The primary distinction lies in
their intended purpose and the data format that supports it:

* *[Export](#export-current-sheet) & [Import](#import-to-current-sheet):* These
   are older functions intended to provide a simple way to transfer the content
   of a single sheet into or out of the application. The exported data contains
   only a single sheet's visual and mathematical content: no sheet name.
* *[Backup](#backup-stored-sheets) & [Restore](#restore-from-backup):* These
   functions are designed for archiving and recovering a collection of stored
   sheets, distinguished by their names. The backup file contains the data from
   the sheets *together with their names* as found in your stored sheets list.

Thus, while the Restore function can read an exported data string (by assigning
it a made-up name), the Import function *cannot* read a backup file. In general,
use *Import* for files created by *Export*, and *Restore* for files created by
*Backup* (although *Restore* will also work with files created by *Export*).

##### Export current sheet

This function allows you to export the contents of the currently displayed sheet
as a JSON text string. This is useful for sharing the sheet's data with other
users or for use in other applications:

1.  From the *Stored Sheet* menu, select *"Export current sheet."*
2.  A dialog box will appear containing JSON that represents the complete data
    of your sheet. This string can be very long (> 1MB) and may take a moment to
    load.
3.  You can then use your system's standard copy function (typically Ctrl+C or
    Command+C) to select and copy text from the dialog window.

*Note:* The text string in the export window is a read-only representation of
your sheet. Modifying the text within this dialog box will not alter the
originally displayed sheet. The exported data is designed to be imported later
or shared.

##### Import to current sheet

This function allows you to load sheet data from a valid JSON string by pasting
it into a text window. This is typically used to display a sheet that was
previously exported from *Group Explorer*:

1.  From the *Stored Sheet* menu, select *"Import to current sheet."*
2.  A dialog box will appear with a text field.
3.  Paste your JSON string into this field. (This string may be very long).
4.  Confirm the action to proceed.

*Important Warning:* The data from the imported JSON string will immediately
 overwrite the contents of the currently displayed sheet, and this action cannot
 be undone. Please ensure that you wish to replace the current sheet before
 confirming.

##### Backup stored sheets

This feature allows you to create a safety copy of your stored sheets, which can
be saved as a file or copied for pasting into other applications:

  1.  From the *Stored Sheets* menu, select the option *"Backup stored sheets."*
  2.  A dialog box will appear, listing all your stored sheets. Check the boxes
      next to the names of the sheets you wish to include in the backup. You can
      select individual sheets or choose all.
  3.  Select your preferred method for saving the backup.
    *   *Save as a File:* This option saves the backup directly to your device.
        You will be prompted to choose a name for the backup file, but for
        security reasons the location is constrained to be wherever your browser
        is configured to place downloads.
    *   *Copy to Clipboard:* This option copies the backup data to your device's
        clipboard. You can then paste it into a text document, email, or other
        location.
  4.  The backup will be generated based on your chosen method.

##### Restore from backup

This feature allows you to recover your stored sheets using a previously created
backup file or data stored in your clipboard:

  1. From the main *Stored Sheets* menu, select the option *"Restore from backup."*
  2. A dialog box will appear, prompting you to choose your backup source.
      * *Restore from File:* Choose this option to import data from a backup
        file saved on your device. A file browser will open for you to locate
        and select the correct file.
      * *Restore from Clipboard:* Choose this option if you have previously
        copied backup data to your clipboard. The system will use the data
        currently available there.
  3. After the backup data is loaded, a list of all the sheet names contained
     within the backup will be displayed.
  4. Select the individual sheets you wish to restore, or choose to restore all
     of them. Once your selection is made, confirm the action to complete the
     process. The selected sheets will be restored to your stored sheets list.

*Note:* Restoring a sheet with the same name as an existing one will overwrite
the current version. Be sure you want to replace it before confirming!
...
To resize a node, first select it by clicking [tapping] it, or by selecting
"Resize" from the context menu. Once selected, a light blue outline will appear
around the node, and until it is deselected subsequent operations will affect
only that node. Using a mouse you can resize the node by dragging the grab icon
in the lower-right corner; on a touch platform you can drag outside of any of
the node's edges to resize it in that direction. (If you start the drag from
inside the node it will reposition the element without changing its size.) 
