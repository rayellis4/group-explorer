...
function onPointerDown( event ) {
	...
	if ( this._pointers.length === 0 ) {
	     ...
	     // Removed to allow CayleyDiagramViewUI to hear click too
...
function onMouseMove( event ) {
   ...
   // Mouse only drags with button down
   // (moving off-screen, releaseing button, and returning doesn't continue drag)
   if (event.buttons === 0) {
      this.state = _STATE.NONE
